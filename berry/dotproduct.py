...
def dot(nk: int, j: int, neighbor: int, jNeighbor: Tuple[np.ndarray]) -> None:
    ...
    if m.noncolin:  # Noncolinear case
        for band0 in range(initial_band,m.nbnd):
            ...
            for band1 in range(initial_band,m.nbnd):
                ...
                b0 = band0 - initial_band
                b1 = band1 - initial_band
...
def run_dot(npr: int = 1, logger_name: str = "dot", logger_level: logging = logging.INFO, flush: bool = False):
    ...
    ########################################################################### 
    DPC_SIZE = m.nks * 2 * m.dimensions * number_of_bands * number_of_bands
    DPC_SHAPE = (m.nks, 2 * m.dimensions, number_of_bands, number_of_bands)
    ...
    logger.info(f"\tWill use bands from {initial_band} to {final_band}")
    logger.info(f"\tTotal number of bands to be used: {number_of_bands}\n")
