...
def get_fermi_delta_ea_grad_ea(grad: Gradient, eigen_array: np.ndarray, conduction_band: int, ) -> Tuple[np.ndarray, np.ndarray]:
    grad_dea = np.zeros((2, d.nkx, d.nky, conduction_band + 1, conduction_band + 1), dtype=np.complex128)
    delta_ea = np.zeros((d.nkx, d.nky, conduction_band + 1, conduction_band + 1))
...
def run_shg(conduction_band: int, npr: int = 1, energy_max: float = 2.5, energy_step: float = 0.001, broadning: complex = 0.01j, logger_name: str = "shg", logger_level: int = logging.INFO, flush: bool = True):
    ...
    GAMMA_SHAPE             = (d.nkx, d.nky, conduction_band + 1, conduction_band + 1)
    ...
    signalfinal                 = np.load(os.path.join(d.workdir, "signalfinal.npy"))                    # Not used
    ...
    # 5. SAVE OUTPUT
    ###########################################################################
    with open(os.path.join(d.workdir, "sigma2r.dat"), "w") as sigm:
