"""
Shared basis-rotation machinery used by both ``degenrotation`` (a priori,
energy-driven) and ``basisrotation`` (a posteriori, cluster-driven).

Contains the pure rotation primitives and per-zone processing pipeline:
wavefunction I/O, overlap matrices, Procrustes/SVD rotation, holonomy
correction, iterative gauge refinement, BFS rotation propagation, connected
zone construction, and per-zone orchestration.  Every function takes all of
...
def _available_memory_bytes() -> int:
    ...
    Returns 0 only if psutil raises unexpectedly; callers treat 0 as
    'unknown' and apply a conservative hard limit in that case.
    """
    try:
...
def _correct_zone_holonomy(
    ...
    if len(plaquettes) < min_plaquettes:
        logger.info(
            f"\t    [Holonomy] Only {len(plaquettes)} plaquette(s) < min={min_plaquettes} — skipping."
        )
        return 0
...
def _process_zone_suffix(
    ...
    when the boundary edges are already known from the calling context).
    Returns:
    ...
    N = len(bands)
    n_holo_sweeps = 0
    # When a wfc cache is available all iterative modifications go to RAM;
    # a single flush to disk happens at the very end of this function.
    defer_disk = wfc_cache is not None
        ...
        if spread > 0.1:
            ...
            if holonomy_correction and not (spread > 0.5 and worst_sigma < 0.6):
                ...
                n_holo_sweeps = _correct_zone_holonomy(
                    ...
                    tol=holonomy_tol,
                    wfc_cache=wfc_cache,
                    min_plaquettes=holonomy_min_plaquettes,
                    write_to_disk=not defer_disk,
                )
                logger.info(f"\t  [Option B] Done: {n_holo_sweeps} sweep(s)")
            elif spread > 0.5 and worst_sigma < 0.6:
                logger.warning(
                    f"\t  WARNING: severe topological discontinuity "
                    f"(spread={spread:.3f}, worst_sigma={worst_sigma:.3f}). "
                    f"Holonomy correction skipped; refinement may not converge."
                )
    ...
    # Flush all zone wfc files from RAM cache to disk in one pass, replacing
    # the per-iteration disk writes that were skipped when defer_disk=True.
    if defer_disk:
        for nk_ in zone:
            for b in bands:
                arr = wfc_cache.get((nk_, b))
                if arr is not None:
                    _save_wfc(arr, nk_, b, noncolin, wfcdir, compress)
...
def _process_zone(
    ...
    # Pre-compute boundary edges here so _process_zone_suffix does not repeat
    # the work; passed via pre_boundary_edges.
    boundary_edges: list = []
    for nk in zone:
        for j in range(n_dir):
            nb = neighbors[nk, j]
...
# ---------------------------------------------------------------------------
# Zone construction (shared Phase 2)
# ---------------------------------------------------------------------------

def build_zones(degen_at_k: dict, neighbors: np.ndarray, n_dir: int) -> list:
    """Form connected degenerate zones from per-k-point degenerate groups.

    Parameters
        degen_at_k : dict {nk: [frozenset(bands), ...]}
            Degenerate band groups present at each k-point.  Bands are original
            (machine) band indices, matching the wfc-file band space.
        neighbors : ndarray (nks, n_dir)
            Neighbour table; -1 marks a missing neighbour (open BZ boundary).
        n_dir : int
            Number of neighbour directions (2 * dimensions).

    Returns
        zones : list of (frozenset_of_bands, set_of_kpoints)
            One entry per connected zone, ordered by ascending band-group size
            so a sub-zone ({0,1}) is processed before the enclosing super-zone
            ({0,1,2}) that sits on top of it.  A zone of a single k-point is the
            "isolated" case; size >= 2 propagates across the zone.

    Only k-points present in ``degen_at_k`` ever enter a zone, so downstream
    rotation never writes a k-point that was not flagged as degenerate.
    """
    # Every signature that appears as the exact degenerate group at some k-point.
    actual_sigs = {grp for groups in degen_at_k.values() for grp in groups}

    # A k-point with signature T contributes to every sub-signature S subset of
    # T that is itself a real signature elsewhere in the BZ.  This bridges gaps
    # where the degeneracy dimension increases (a doublet region flanked by a
    # triplet region): the doublet zone then includes the triplet k-points,
    # keeping the zone connected instead of split into disconnected pieces.
    sig_kpoints = defaultdict(set)
    ...
    # Process signatures in ascending size order so a sub-zone is fully rotated
    # before the enclosing super-zone runs on top of it.
    zones = []  # list of (frozenset_of_bands, set_of_kpoints)
