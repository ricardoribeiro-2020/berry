...
bands up to the highest one are used. The minimum band is set with the option \texttt{-mb}.
...
Its initial value is $1$ (so the first iterations rely on the dot product alone), and it is swept down to its minimum value of $0$ in steps of size \texttt{step} (default $0.5$, giving the sweep $1 \to 0.5 \to 0$).
...
where $\sigma$ is the local one-step energy dispersion (the $99.9$th percentile of $|E(k+1)-E(k)|$) and $E_\text{accept}\approx 3\sigma$ is a loose gross-jump guard: a candidate on the far side of an energy wall (a genuine inter-band gap) scores $0$ and is never joined.
...
If there are not enough points to fit the curve, the nearest neighbor's energy is used.
The gross-jump guard $E_\text{accept}$ is deliberately not keyed to the band gap: gating on the gap once shattered the graph, so energy is kept out of the graph edges and used only through $f_p(E_q)$ and the a-posteriori repair described below.
