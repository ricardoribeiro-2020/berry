...
Iterative refinement cannot reduce the spread; the two gauge-repair options
described below must act before refinement.


\subsection*{Gauge repair --- Option~B: holonomy correction}

\textbf{Trigger:} boundary spread $> 0.1$ \emph{and not} a topological
discontinuity (see Option~C below).
...
\subsection*{Gauge repair --- Option~C: topological zone splitting}

\textbf{Trigger:} boundary spread $> 0.5$ \emph{and} the minimum boundary
singular value $\sigma_{\min}^{\text{worst}} < 0.6$.

\textbf{Physical meaning:} There is a genuine topological discontinuity in the
U($N$) gauge bundle over the Brillouin Zone (e.g.\ a Berry phase of $\pi$
or a non-zero Chern number).
No smooth gauge exists across the whole zone.
The worst boundary edge marks the physical branch cut.

\textbf{Algorithm:}
\begin{enumerate}
  \item The zone k-point $\pmb{k}^*$ with the worst boundary alignment
        (lowest $\sigma_{\min}$) is identified.
  \item All internal zone edges adjacent to $\pmb{k}^*$ are considered as
        candidate cut edges.
        For each candidate $(\pmb{k}^*, \pmb{k}_c)$, a BFS is run from the
        original root $\pmb{k}_{\mathrm{root}}$ with that edge removed.
        If $\pmb{k}^*$ becomes unreachable, the candidate is a valid cut.
  \item Among valid candidates, the edge with the \emph{lowest} internal
        overlap $\sigma_{\min}(\pmb{k}^*, \pmb{k}_c)$ is chosen as the
        physical branch cut, because it marks the point where the two-sided
        wavefunctions are most nearly orthogonal.
  \item The zone is split into \textbf{zone$_a$} (contains
        $\pmb{k}_{\mathrm{root}}$, already correctly rotated) and
        \textbf{zone$_b$} (contains $\pmb{k}^*$, gauge from the original BFS
        is incorrect).
  \item Phases~3 and~4 are repeated independently on zone$_b$ to give it a
        fresh, self-consistent gauge anchored to its own best external
        reference.
  \item Both zone$_a$ and zone$_b$ then proceed independently through the
        boundary check and iterative refinement.
\end{enumerate}

\textbf{Effect:} Each sub-zone converges cleanly.
The physical Berry-phase discontinuity at the cut edge is preserved, and
downstream programs (dot product, Berry connections) see a genuine gauge
discontinuity that correctly contributes to the Chern number integral.
The file \emph{degenzones.npy} assigns the two sub-zones distinct
zone identifiers.
...
Writes always go to both RAM and disk to maintain consistency.
The cache is skipped automatically when its estimated memory footprint would
exceed 80\% of available RAM (or 4~GB when the available memory cannot be
determined).
...
If an Option~C split occurs, the two sub-zones are assigned distinct zone
identifiers; downstream programs iterate over zone identifiers
transparently.
...
\hline
  ...
  \texttt{-no-holonomy}              & Disable Option~B holonomy correction \\
