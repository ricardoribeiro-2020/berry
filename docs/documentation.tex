...
\hline
  ...
  \texttt{-ethr}                     & Energy threshold for degeneracy detection \\
                                     & in eigenvalue units (default: $5\times10^{-3}$) \\
...
\section{Basis rotation}
 ...
 \begin{description}
   \item[Phase 1 --- read the flagged points.] Each row of \emph{degeneratefinal.npy} is a
     ...
     the grouping threshold \texttt{-gethr} (default $0.01$~eV) is treated as non-degenerate and
...
\begin{table}[h]
 ...
 \hline
  ...
  \texttt{-np}                  & Number of processes to use (default: 1) \\
  \texttt{-gethr eV}            & Energy-coherence threshold for grouping\\
                                & flagged bands into one subspace (default: 0.01;\\
...
\begin{enumerate}
    ...
    \item \textbf{Final revalidation, degenerate-group completion and bijection.} Every attributed slot is re-validated by the energy-continuity criteria (in-loop only the doubtful points are checked, which could let one side of a label swap ship as clean); an isolated failure on a band whose overall score exceeds $0.96$ is graded as a partial discontinuity rather than a mistake. Inside a near-degenerate group of bands (energies within a few meV) the individual band label is a gauge choice, so the group's slots are completed by a within-group bijection. The full attribution is then made a per-k permutation (each original band used in at most one slot): duplicated bands keep their best copy and the losers move to a missing band by best energy fit. (The degenerate-group completion and the bijection also run as part of every in-loop validation, not only here.)
    \item \textbf{Wavefunction-evidence pass.} The energy criteria cannot see a label swap between near-parallel bands (a few-meV error passes every energy gate), so cells whose attribution has no dot-product support are exchanged when the swap is dot-product-favoured, and the zero-support cells that remain are demoted, so that a silently swapped band cannot be reported clean.
\end{enumerate}
