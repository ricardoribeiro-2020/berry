...
dot products are computed by \texttt{dot}, it is possible to run the optional
...
(or numerically indistinguishable) energy eigenvalue.
...
\subsection*{The problem of degenerate subspaces}
...
where $\varepsilon$ is the energy threshold (default $10^{-4}$, in the same
...
\hline
  ...
  \texttt{-ethr}                     & Energy threshold for degeneracy detection \\
                                     & in eigenvalue units (default: $10^{-4}$) \\
...
\section{Basis rotation}
 ...
 never changed. The rotated wavefunctions \textbf{overwrite the \emph{.wfc} files in place}
 (there is no \emph{.wfc1} file). Both colinear and non-colinear calculations are supported: for
