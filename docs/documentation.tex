...
\chapter{Introduction}\label{ch:introduction}
 ...
 This guide gives a general introduction to the \textbf{berry} suite of programs, including installation and running.
...
It can be used in one, two or three dimension materials, with noncollinear or no-spin calculations.
...
The \textbf{berry} suite is coordinated by Ricardo Mendes Ribeiro (University of Minho, Braga, Portugal),
...
\begin{itemize}
 ...
 and AI work;
 \item Daniel Sousa (Braga, Portugal) for implementing the noncollinear code
...
All files for installation can be found there.
...
We acknowledge the Fundação para a Ciência e a Tecnologia (FCT)
...
\subsection{Patch for noncollinear calculations}

As for the time of this version, the {\sc Quantum ESPRESSO} suite of programs
does not include a script to correctly extract the spinors of the noncollinear calculation
in real space.

The script that should do it is \verb*|wfck2r.x|, but it only writes the first function of the spinor.
In order to use the \textbf{berry} correctly in noncollinear calculations,
...
This version of \textbf{berry} only supports the DFT package {\sc Quantum Espresso}, version v.7.5 or higher.
So a working {\sc Quantum Espresso} installation has to be in the system, and must be in the \$PATH.

This version was tested with python 3.12, but likely works with other versions of python3.
...
\begin{verbatim}
 preprocess
 wfcgen
 degenrot (optional; required for noncollinear calculations)
 dot
 cluster
 basis (optional)
 r2k
 ...
 shg
\end{verbatim}
and the parameter is dependent on which script is being run.
...
The flowchart in the next page explains how \textbf{berry} works.

First a preprocessing script is run which runs the DFT calculations, reads data from those
calculations, and saves it in files ready to be read by the following scripts.
...
One can go from one unit cell to any other one by applying the translation
\begin{equation}
...
\end{figure}
Figure \ref{fig:orderbands} illustrates the problem.
...
in such a way that analyticity is guaranteed.
...
\section{The problem of degenerate subspaces}

At a generic k-point in the Brillouin Zone (BZ), the eigenstates of the
Kohn-Sham Hamiltonian (see equation \ref{eq:Schrodinger}) are non-degenerate
and uniquely determined up to an overall complex phase.
However, at high-symmetry points, or along lines of high symmetry, two or more
bands can share the same eigenvalue $E_{\pmb{k}}$.
In that case, any linear combination of the corresponding eigenstates is also
an eigenstate with the same energy.
The DFT solver (here {\sc Quantum Espresso}) returns an arbitrary orthonormal
basis for this degenerate subspace, which in general bears no relation to the
basis chosen at neighbouring k-points.

As a consequence the Bloch factors $u_{\pmb{k}s}(\pmb{r})$, which should vary
smoothly with $\pmb{k}$ within each band $s$, can have an arbitrary
orientation at every degenerate k-point.
This discontinuity manifests as anomalously small dot products
$|\langle u_{\pmb{k}s} | u_{\pmb{k}'s}\rangle|$ between a degenerate k-point
and its neighbours, even when the two k-points belong to the same band.

The goal of \texttt{degenrot} is to rotate the wavefunction basis at every
degenerate k-point so that it aligns as closely as possible with the basis of
the surrounding non-degenerate region, restoring continuity before the dot
...
\paragraph{Phase 1 --- Detection of degenerate subspaces.}

For every k-point $\pmb{k}$ and every pair of bands $(s, s')$ with $s < s'$,
the script checks whether

\begin{equation}
  |E_{\pmb{k},s} - E_{\pmb{k},s'}| < \varepsilon
  \label{eq:degen_threshold}
\end{equation}

where $\varepsilon$ is the energy threshold (default $5\times10^{-3}$, in the same
units as the eigenvalues).
A union-find algorithm \cite{Cormen2009} groups all mutually-degenerate bands at
each k-point into a degenerate subspace of dimension $N \ge 2$.


\paragraph{Phase 2 --- Formation of degenerate zones.}

For each unique band-group signature (i.e.\ each distinct frozenset of band
indices), the set of k-points that share that degeneracy is collected and its
connected components are found by breadth-first search (BFS) over the
neighbour graph from \emph{neighbors.npy}.

Each connected component is called a \emph{degenerate zone}: a contiguous
region of k-space in which the same $N$ bands are degenerate.
A single calculation may contain many independent zones, each treated
separately.


\paragraph{Phase 3 --- Reference selection.}

To define an unambiguous target basis for a zone, \texttt{degenrot} searches
for a \emph{boundary k-point}: a k-point that belongs to the zone and has at
least one neighbour outside the zone where the $N$ bands are
\emph{non}-degenerate (i.e.\ where the minimum pairwise energy gap exceeds
$\varepsilon$).
Among all such boundary–neighbour pairs the one with the largest minimum
pairwise energy separation is chosen as the reference, because it provides the
most unambiguous basis.

If no non-degenerate external neighbour exists (the zone is \emph{fully
isolated}), an arbitrary k-point in the zone is chosen as the root.
A warning is written to the log file and the zone is given an internally
consistent basis, but the absolute orientation of that basis is arbitrary.


\paragraph{Phase 4 --- Propagation of the basis rotation by BFS.}

Starting from the boundary reference k-point, the script propagates the
rotation outward through the zone via BFS.
At each k-point $\pmb{k}$ (with BFS parent $\pmb{k}_{\mathrm{ref}}$, which
has already been rotated):

\begin{enumerate}

\item The $N \times N$ Bloch-phase-corrected overlap matrix is computed:
\begin{equation}
  M_{ij} = \langle u^{\mathrm{ref}}_i | u^{\mathrm{cur}}_j \rangle
          = \frac{1}{N_r}\sum_{\pmb{r}}
            e^{i(\pmb{k}-\pmb{k}_{\mathrm{ref}})\cdot\pmb{r}}\,
            u^*_{\pmb{k}_{\mathrm{ref}},i}(\pmb{r})\,
            u_{\pmb{k},j}(\pmb{r})
  \label{eq:overlap_matrix}
\end{equation}
where $N_r$ is the number of real-space grid points, the sum runs over all
grid points $\pmb{r}$, and $i,j = 1,\ldots,N$ index the bands in the
degenerate subspace.

For non-collinear (NC) calculations, each Bloch factor is a two-component
spinor $u_{\pmb{k}s} = (u_{\pmb{k}s}^{\uparrow}, u_{\pmb{k}s}^{\downarrow})$
and the inner product sums over both spin components:
\begin{equation}
  M_{ij} = \frac{1}{N_r}\sum_{\pmb{r}} e^{i(\pmb{k}-\pmb{k}_{\mathrm{ref}})\cdot\pmb{r}}
            \left[
              u^{*\uparrow}_{\pmb{k}_{\mathrm{ref}},i}(\pmb{r})\,
              u^{\uparrow}_{\pmb{k},j}(\pmb{r})
            + u^{*\downarrow}_{\pmb{k}_{\mathrm{ref}},i}(\pmb{r})\,
              u^{\downarrow}_{\pmb{k},j}(\pmb{r})
            \right]
  \label{eq:overlap_matrix_nc}
\end{equation}

\item The Singular Value Decomposition (SVD) \cite{Golub2013} of $M$ is computed:
\begin{equation}
  M = U\,\Sigma\,V^{\dagger}
  \label{eq:svd}
\end{equation}
where $U$ and $V$ are $N \times N$ unitary matrices and
$\Sigma = \mathrm{diag}(\sigma_1, \sigma_2, \ldots, \sigma_N)$
with $\sigma_i \ge 0$.

\item The optimal rotation matrix is obtained from the
orthogonal Procrustes problem \cite{Schonemann1966}: find the $N \times N$
unitary matrix $R$ that maximises $\mathrm{Re}\,\mathrm{Tr}(M R)$.
...
\item The new Bloch factors at $\pmb{k}$ are
\begin{equation}
  u^{\mathrm{new}}_{\pmb{k},j}(\pmb{r}) = \sum_{i=1}^{N} R_{ij}\, u_{\pmb{k},i}(\pmb{r}),
  \qquad j = 1, \ldots, N
  \label{eq:rotation_application}
\end{equation}
and they overwrite the corresponding \emph{.wfc} files in-place.
For NC calculations the same matrix $R$ is applied to each spin component
independently.

\end{enumerate}


\subsection*{Singular values as a quality measure}

The singular values $\sigma_i$ of the overlap matrix $M$ are the cosines of
the \emph{principal angles} \cite{Bjorck1973} between the $N$-dimensional
subspace spanned by the reference basis and the $N$-dimensional subspace
spanned by the current basis:

\begin{equation}
  \sigma_i = \cos\theta_i, \qquad i = 1, \ldots, N
\end{equation}

where $0 \le \theta_1 \le \theta_2 \le \ldots \le \theta_N \le \pi/2$ are
the principal angles.
If both bases span exactly the same subspace (as is the case when the
degeneracy is clean and the k-spacing is small), all singular values equal
one: the two subspaces coincide and the Procrustes rotation is exact.

A singular value significantly below one indicates that one direction in the
current subspace is tilted away from the reference subspace, which means the
degenerate bands are mixing with states outside the chosen $N$-dimensional
model.
The minimum singular value $\min_i\sigma_i$ is the most sensitive diagnostic:
it measures the worst-case alignment across all $N$ directions.
Typical values above $0.99$ indicate a reliable rotation; values below $0.9$
should prompt the user to check whether additional bands ought to be included
in the degenerate group.

Note that a permutation of the basis vectors, a sign flip, or any other
unitary rearrangement of the current basis \emph{within} the same subspace
gives singular values all equal to one, because the overlap matrix $M$ is
itself unitary in that case.
A small singular value therefore strictly means the two subspaces are
genuinely different, not merely expressed in different bases.


\subsection*{Boundary consistency check}

After the BFS rotation has been applied to all k-points in the zone, the
script checks whether the resulting gauge is \emph{globally consistent} across
the zone boundary.
For every zone k-point $\pmb{k}$ that has a non-degenerate external neighbour
$\pmb{k}'$ (i.e.\ a grid neighbour outside the zone where the $N$ bands are
split), the minimum singular value $\sigma_{\min}(\pmb{k},\pmb{k}')$ is
computed from the overlap matrix between the rotated basis at $\pmb{k}$ and
the unmodified basis at $\pmb{k}'$.

The \emph{boundary spread}
\begin{equation}
  \Delta\sigma = \max_{\text{boundary edges}}\sigma_{\min} -
                 \min_{\text{boundary edges}}\sigma_{\min}
\end{equation}
measures how uniformly the rotated zone aligns with its external neighbourhood.
A large spread indicates that the BFS tree imposed a path-dependent gauge
choice, so that different boundary k-points align well or poorly depending on
which BFS path was used to reach them.

An important invariant: boundary singular values are \emph{gauge-invariant}.
Iterative refinement cannot reduce the spread; the holonomy correction
described below must act before refinement when the spread is large.


\subsection*{Gauge repair --- holonomy correction}

\textbf{Trigger:} boundary spread $> 0.1$ and the zone contains at least
\texttt{-holonomy-\allowbreak min-\allowbreak plaquettes} internal plaquettes (default: 2).
When the spread is very large ($> 0.5$) and the worst boundary singular value
is below 0.6 --- indicating a topological discontinuity --- the correction is
skipped and a warning is logged, because the holonomy cannot be reduced in
that case (see below).

\textbf{Physical meaning:} The BFS tree forced a path-dependent gauge choice.
No topological obstruction prevents a smooth consistent gauge, but the BFS
did not find it.

\textbf{Algorithm:} The script walks all \emph{elementary plaquettes},
i.e.\ minimal 4-cycles $\pmb{k}_0 \to \pmb{k}_1 \to \pmb{k}_2 \leftarrow
\pmb{k}_3 \leftarrow \pmb{k}_0$ in the zone graph, and computes for each
plaquette the \emph{holonomy}

\begin{equation}
  H = R_{01}\, R_{12}\, R_{23}^{\dagger}\, R_{30}^{\dagger}
\end{equation}

where each $R_{ij}$ is the Procrustes rotation of equation~\eqref{eq:procrustes}
evaluated between the current Bloch factors at $\pmb{k}_i$ and $\pmb{k}_j$.
A perfectly consistent gauge gives $H = \mathbf{1}$; the departure from
identity is the \emph{loop defect}.

The defect is distributed by applying fractional corrections
$H^{-1/4}$, $H^{-1/2}$, $H^{-3/4}$ to the three non-anchor vertices of the
plaquette.
The fractional power is computed via two successive matrix square roots
(using \texttt{scipy.linalg.sqrtm}) followed by projection onto
$\mathrm{U}(N)$.
The procedure iterates over all plaquettes until the maximum holonomy defect
$\|H - \mathbf{1}\|_F$ falls below a tolerance (default~$10^{-4}$) or a
maximum number of sweeps is reached (default~$20$).

\textbf{Early exit on topological holonomy:} If the maximum defect does not
decrease between two consecutive sweeps (relative change $< 10^{-8}$), the
zone carries an \emph{irreducible} Berry-phase holonomy: the total holonomy
around the zone boundary is a topological invariant that cannot be eliminated
by local plaquette corrections.
The correction exits immediately with a warning; iterative refinement proceeds
but may not fully converge.

\textbf{Effect:} The internal gauge consistency improves, giving the
subsequent iterative refinement a better starting point and allowing it to
converge more quickly.
The boundary spread (being gauge-invariant) is not changed by this step.


\subsection*{Iterative gauge refinement}

After any gauge repair (or directly if the spread is small), all zone
k-points are refined simultaneously by a multi-neighbour Procrustes sweep.

Each sweep (a Gauss-Seidel pass, or a Jacobi pass when parallelism is
enabled) re-rotates every zone k-point to best align with \emph{all} its
valid neighbours simultaneously: both zone-internal neighbours (also being
refined) and external non-degenerate neighbours (fixed anchors that prevent
global gauge drift).
The overlap matrices from every neighbour are summed before the Procrustes
step so all neighbours vote on the rotation:

\begin{equation}
  M_{\text{total}}(\pmb{k}) = \sum_{\pmb{k}' \in \mathcal{N}(\pmb{k})}
  M(\pmb{k}', \pmb{k})
\end{equation}

where $\mathcal{N}(\pmb{k})$ includes all zone-internal neighbours and all
non-degenerate external neighbours.

Convergence is declared when $\max_{\pmb{k}} \|R_{\pmb{k}} - \mathbf{1}\|_F$
falls below the refinement tolerance (default~$10^{-4}$) or the maximum
iteration count is reached.
The iteration limit scales with zone size:
$\max(N_{\mathrm{base}},\, N_{\mathrm{base}} + 3\,|\text{zone}|)$,
capped at a hard maximum (defaults: base~50, cap~500).

\textbf{Stagnation early exit.}
If the residual is still more than ten times the tolerance but has improved by
less than a factor of two over the last 30 iterations, the solver has reached a
topological floor and further iterations are wasteful.
Refinement exits early with a warning; the final rotations are still written
and the zone is marked in the log.

\paragraph{Anderson acceleration.}
Each Gauss-Seidel sweep is followed by an Anderson mixing
step~\cite{Walker2011} that fits a least-squares linear combination of the
last $m$ sweep residuals (default $m = 10$) to predict the fixed point and
extrapolates the rotation matrices in that direction.
Each $N \times N$ block of the extrapolated flat state vector is
projected back onto $\mathrm{U}(N)$ via SVD\@.
The safety criterion rejects any Anderson step that would exceed five
times the plain Gauss-Seidel step.
This acceleration converts the slow linear convergence ($\rho \approx 0.97$
per iteration without acceleration) into effective superlinear convergence,
typically reducing the required iteration count by a factor of 5--10.

\paragraph{Jacobi parallel sweep.}
When the \texttt{np} option is greater than one and the in-memory
wavefunction cache is active, the Gauss-Seidel sweep is replaced by a
\emph{Jacobi} parallel sweep: a snapshot of the current wavefunction state
is taken at the start of each sweep, all k-points compute their rotation
independently (using the snapshot), and the updates are applied after all
workers have finished.
This parallelises the most expensive part of the refinement at the cost of
approximately twice as many iterations to converge (Jacobi converges at
$\rho \approx 0.97$ vs.\ $\rho^2 \approx 0.94$ for Gauss-Seidel).
The break-even is four or more workers; beyond that Jacobi gives a net
speedup.
...
Nonetheless, if the weights $\omega_e$ of all edges $e$ are the same ($\forall\, e\in E\; : \;\omega_e = \omega$) or the edges express a binary relation (may exist or not), the graph is unweighted.
...
\hline
  ...
  \texttt{-o file\_path}        & Name of output log file\\
                                & Extension will be .log regardless of user input \\
  \texttt{-flush}               & Flush the output to stdout \\
...
All log files are created in directory \verb|log|.
 ...
 It can be changed in the input file, but is not advisable,
...
\end{figure}
 ...
 for the calculation of the linear and second harmonic conductivity.
 ...
 This convention is enforced: if \verb|nkx, nky, nkz| indicate that the material varies along an axis
 other than $x$ (1D) or outside the $xy$ plane (2D), \verb|preprocessing| logs an error and stops.
 ...
 For reference, we show below two possible samplings for a 2D hexagonal material
...
To sample the area equivalent to the BZ using a rectangle, one can choose the one shown
...
\begin{figure}[ht]
 ...
 \caption{Rectangle with an equivalent area to the BZ for a 2D hexagonal lattice.}
...
$\vec{b}_3 = \dfrac{2\pi}{a}\left( \vec{k}_x + \vec{k}_y - \vec{k}_z\right) $\\[0.2cm]
\hline
\end{tabular}
\bigskip

One possible choice of sample is to choose the following three directions in k-space:
\begin{align*}
 \hat{k}_1 &= (0,0,1)\\
...
\end{align*}
 with sizes:
\begin{align*}
  |\vec{k}_1| &= \dfrac{4\pi}{a} \\
  |\vec{k}_{2,3}| &= \dfrac{2\pi}{a}\sqrt{2}
\end{align*}
...
\bigskip
 ...
 It is important to obtain and look at a normal band diagram of the material before choosing
...
\bigskip
 ...
 \subsection{Discarding a region of the supercell}
 ...
 there is some noise.
 Knowing this, it is possible to discard part of the supercell, removing that noise without
 compromising the results.

 Note that this does not make the supercell smaller: the wavefunctions are simply set to zero
 on the grid points of the selected region.
 The lattice vectors, the real space grid (\verb|nr1|, \verb|nr2|, \verb|nr3|) and the size of
 the wavefunction files are unchanged, so no memory or disk space is saved by using it.
 ...
 \verb|z1| and \verb|y1| are used to define the initial limit of the region to be discarded, and 
 should have a value between 0 and $a_3$ and $a_2$, respectively.
 Then \verb|deltaz| and \verb|deltay| define the width of the region to be discarded, and should have 
 a value between 0 and $a_3$ and $a_2$, respectively.
 The default values are 0, meaning no region is discarded.
 \verb|deltaz| is only taken into account in 1D and 2D calculations, and \verb|deltay| only in 1D
 calculations; outside of those cases the parameters are silently ignored.
 The region that was discarded, and the number of grid points it contains, are written to the
 logs of \verb|preprocess| and of \verb|generatewfc|.
...
\]
 ...
 Each of the cases will discard a region of the supercell either from the middle (see figure
 \ref{fig:vcut}(\subref{fig:vcutcenter})) or from its sides (see figure \ref{fig:vcut}(\subref{fig:vcutsides})).
...
\begin{figure}[ht]
    ...
    \begin{subfigure}{.5\textwidth}
        ...
        \label{fig:vcutsides}
    \end{subfigure}
    \caption{When \(y_1 + \Delta y \leq a_2\), (\subref{fig:vcutcenter}) the region discarded is located
    on the center of the supercell.
    Otherwise, (\subref{fig:vcutsides}) the region discarded will be split into two sides.}
    \label{fig:vcut}
\end{figure}
...
\hline
 origin of k-points (3 real numbers)           & \texttt{k0} \\
 number of k-points in each direction (integer)& \texttt{nkx, nky, nkz} \\
 step of the k-points (real number)            & \texttt{step} \\
 number of bands to be calculated (integer)    & \texttt{nbnd} \\
 \hline
 number of processors                         & \texttt{npr = 1} \\
 dft directory                                & \texttt{dftdirectory = 'dft/'} \\
 scf file name                                & \texttt{name\_scf = 'scf.in'} \\
 directory where wavefunctions will be saved  & \texttt{wfcdirectory = 'wfc/'} \\
 point in real space where all phases match   & \texttt{point = 1.178097} \\
 software for DFT                             & \texttt{program = 'QE'} \\
 unique reference of run                      & \texttt{refname} = date and time \\
 first direction in k-space                   & \texttt{kvector1 = 1.0 0.0 0.0} \\
 second direction in k-space                  & \texttt{kvector2 = 0.0 1.0 0.0} \\
 third direction in k-space                   & \texttt{kvector3 = 0.0 0.0 1.0} \\
 last band NOT accounted for                  & \texttt{wfcut = -1} \\
 initial limit of discarded region             & \texttt{y1, z1 = 0} \\
 width of discarded region                    & \texttt{deltay, deltaz = 0} \\
...
Since wfck2r.x extracts $u_{\bf{k}s}$, that's what is saved.

After extracting the wavefunctions,
 a point in position space is chosen (see table \ref{tab:variables_preprocessing}),
...
The wavefunctions are then saved in the directory chosen to save them
...
The default is to generate all formatted wavefunctions, but for debugging it may be useful
to generate just for a single k-point or a single wavefunction, for which we have the options
...
\hline
  ...
  \texttt{-c}                   & Compress the output files \\
  \texttt{-h}, \texttt{--help}  &\hspace*{2cm} Show a help message and exit \\
  \texttt{-o file\_path}        & Name of output log file\\
                                & Extension will be .log regardless of user input \\
  \texttt{-flush}               & Flush the output to stdout \\
...
% -----------------------------------------------------------------------
\section{Degenerate subspace basis rotation}\label{sec:degenrot}

After the wavefunctions have been extracted by \texttt{wfcgen}, and before the
dot products are computed by \texttt{dot}, it is possible (and convenient) to run the optional
script \texttt{degenrot}.

\begin{verbatim}
  berry degenrot [script options]
\end{verbatim}

This script is optional but recommended whenever the band structure contains
degenerate points, i.e.\ k-points at which two or more bands share the same
(or numerically indistinguishable) energy eigenvalue. For noncollinear calculations,
this is necessary, since there are whole bands degenerate.
Without it, the arbitrary basis chosen by the DFT
solver at those k-points propagates into the dot products and can degrade the
quality of the subsequent band classification by \texttt{cluster}.



\subsection*{Parallelism}

\texttt{degenrot} supports three levels of parallelism, all controlled by the
single \texttt{np} option:

\begin{enumerate}
  \item \textbf{Phase~1 (k-point detection):} the degeneracy check for each
        k-point is independent and dispatched to a process pool
        (\texttt{ProcessPoolExecutor}).

  \item \textbf{Zone-level:} multiple degenerate zones that share the same
        band signature (and thus have disjoint k-point sets) are processed
        simultaneously using separate processes with copy-on-write memory
        sharing (fork start method; Linux and macOS only).

  \item \textbf{Sweep-level (Jacobi):} within a single zone's iterative
        refinement, the per-k-point rotation computation is parallelised
        using a thread pool (\texttt{ThreadPoolExecutor}); requires the
        in-memory wavefunction cache to be active.
\end{enumerate}

These three levels are mutually compatible and engage automatically based on
the available zone structure and cache state.

%
% \subsection*{In-memory wavefunction cache}
%
% By default, \texttt{degenrot} pre-loads all wavefunction files for a zone
% (zone k-points plus their grid neighbours) into a RAM dictionary before
% starting the boundary check and iterative refinement.
% All subsequent reads come from RAM instead of disk, which can reduce I/O
% by a factor of 100 or more for large zones.
%
% When the cache is active, \emph{all writes during holonomy correction and
% iterative refinement are deferred to RAM only}.
% A single disk flush at the end of each zone replaces the thousands of
% per-iteration file writes that would otherwise occur.
% For a zone of 1600 k-points and two non-collinear bands this reduces disk
% traffic per zone from tens of gigabytes (one full write per iteration) to a
% single pass of roughly 35~GB written once.
%
% Available RAM is queried via \texttt{psutil} (a required dependency).
% The cache is skipped automatically when its estimated footprint would exceed
% 80\% of available RAM; a conservative 4~GB hard limit applies only if
% \texttt{psutil} fails at runtime.


\subsection*{Output}

\texttt{degenrot} overwrites the \emph{.wfc} files of the rotated k-points
in-place and saves one additional file \emph{degenzones.npy} in the
\emph{data} directory.
Each row of \emph{degenzones.npy} has the form

\begin{center}
  \verb|[zone_id,  b_1, b_2, ..., b_N,  k-point]|
\end{center}

where \verb|zone_id| identifies the degenerate zone, $b_1 \ldots b_N$ are the
absolute band indices of the degenerate subspace (padded with $-1$ if the
subspace dimension is smaller than the maximum across all zones), and
\verb|k-point| is the index of the k-point.

A log file is written to \emph{log/degenrotation.log} (or the name given with
\texttt{-o}).
The final summary table records, for every zone, the bands involved, the
type of reference (\emph{boundary} or \emph{isolated}), the total number of
k-points in the zone, the number of k-points rotated in Phase~4, the BFS
root k-point, the mean BFS singular value, and the number of holonomy
correction sweeps performed (column \emph{Holo}).

The options for \texttt{degenrot} are shown in table~\ref{tab:options_degenrot}.

\begin{table}[ht]
 \centering
 \caption{Options for script \texttt{degenrot}.}\label{tab:options_degenrot}
\begin{tabularx}{\textwidth}{lX}
\hline
  \texttt{-h}, \texttt{--help}       & Show a help message and exit \\
  \texttt{-ethr}                     & Energy threshold for degeneracy detection in Ry, the units of \emph{eigenvalues.npy} (default: $5\times10^{-3}$~Ry $\approx 68$~meV) \\
  \texttt{-c}                        & Compress the output \emph{.wfc} files \\
  \texttt{-max-ref-iter}             & Base iteration count used to compute the per-zone refinement cap (not itself a maximum: the cap scales with zone size and is capped by \texttt{-ref-iter-cap}) (default: 50) \\
  \texttt{-ref-iter-cap}             & Hard cap on refinement iterations (default: 500) \\
  \texttt{-anderson-m}               & Anderson mixing history window; 0 to disable (default: 10) \\
  \texttt{-ref-tol}                  & Refinement convergence tolerance (default: $10^{-4}$) \\
  \texttt{-no-holonomy}              & Disable holonomy correction \\
  \texttt{-holonomy-max-iter}        & Maximum holonomy correction sweeps (default: 20) \\
  \texttt{-holonomy-tol}             & Holonomy convergence tolerance (default: $10^{-4}$) \\
  \texttt{-holonomy-min-plaquettes}  & Min.\ internal plaquettes to attempt correction (default: 2) \\
  \texttt{-no-cache}                 & Disable in-memory wavefunction cache \\
  \texttt{-np}                       & Number of processes to use (default: 1) \\
  \texttt{-o file\_path}             & Name of output log file. Extension will be .log regardless of user input \\
  \texttt{-flush}                    & Flush the output to stdout \\
  \texttt{-v}                        & Increase output verbosity \\
\hline
\end{tabularx}
...
\hline
  \texttt{-h}, \texttt{--help}  &\hspace*{2cm} Show a help message and exit \\
  \texttt{-np}                  & Number of processes to use (default: 1) \\
  \texttt{-bs}                  & Band-block size for the tiled dot product\\
                                & (default: 0 = auto-size from free RAM) \\
  \texttt{-c}                   & Compress the output files \\
  \texttt{-o file\_path}        & Name of output log file.\\
                                & Extension will be .log regardless of user input. \\
  \texttt{-flush}               & Flush the output to stdout \\
...
It uses data from files \emph{datafile.npy}, \emph{dp.npy}, \emph{eigenvalues.npy} and
\emph{neighbors.npy} to establish continuity between wavefunctions of neighboring k-points.

The optional positional parameter \texttt{Mb} is the maximum band to consider; if omitted, all
bands up to the highest one are used. The minimum band is fixed at preprocessing time (the first
band above the \texttt{wfcut} cutoff), since all the band-indexed files the script reads start there.

 It produces the following files:
 \begin{description}
   \item[\emph{bandsfinal.npy}] stores an array bandsfinal[k,b] = number of the original (DFT) band that is continuous to band b at k-point k.
   \item[\emph{signalfinal.npy}] signalfinal[k,b] is a measure of the quality of the attribution to a band, on the dot-product scale (see table \ref{tab:signals1}).
   \item[\emph{correct\_signalfinal.npy}] the same quality measure on the energy-continuity scale (see table \ref{tab:signals2}).
   \item[\emph{degeneratefinal.npy}] the list of k-points that need a basis rotation.
   \item[\emph{final\_score.npy}] the mean dot-product score of each band.
   \item[\emph{completed\_bands.npy}] the bands that were clustered free of genuine errors (usable directly).
   \item[\emph{final.report}] a human-readable summary, with a per-band status table.
 \end{description}
...
\begin{table}[ht]
  ...
  \hline
  \texttt{-h}, \texttt{--help}  & Show a help message and exit \\
  \texttt{Mb}                   & Maximum band to consider (positional; default: highest band) \\
  \texttt{-np}                  & Number of processes to use (default: 1) \\
  \texttt{-t [0.0-1.0]}         & Tolerance used for graph construction (default: 0.80) \\
  \texttt{-step [0.0-1.0]}      & Step for the alpha sweep (default: 0.5, i.e. 1.0 $\to$ 0.5 $\to$ 0.0) \\
  \texttt{-alpha [0.0-1.0]}     & Initial alpha value (default: 1.0) \\
  \texttt{-o file\_path}        & Name of output log file\\
                                & Extension will be .log regardless of user input \\
  \texttt{-flush}               & Flush the output to stdout \\
  \texttt{-v}                   & Increase output verbosity \\
...
\section{Basis rotation}
 Where two or more bands are degenerate, the individual wavefunctions are only defined up to
 an arbitrary unitary mixing within the degenerate subspace, and the DFT output picks that
 mixing arbitrarily at each k-point. A basis rotation has to be applied to restore continuity
 across k-space.

 For this, \texttt{basis} can be run, after \texttt{cluster}:

 \begin{verbatim}
  berry basis [Mb] [script options]
 \end{verbatim}

 The script \texttt{basis} is the \emph{a-posteriori} twin of \texttt{degenrot}
 (section \nameref{sec:degenrot}): it applies exactly the same subspace-rotation machinery,
 but instead of detecting degeneracies from the eigenvalues it takes its list of degenerate
 k-points from the clustering output \emph{degeneratefinal.npy}. This lets it fix the specific
 points that the band classification flagged as needing a rotation, rather than every energy
 degeneracy in the mesh.

 The algorithm proceeds in the same phases described in section \nameref{sec:degenrot}, and only
 the first (detection) phase differs:

 \begin{description}
   \item[Phase 1 --- read the flagged points.] Each row of \emph{degeneratefinal.npy} is a
     degenerate band pair $[k, b_1, b_2]$. Overlapping pairs that share a band are merged into
     the maximal degenerate group (so a triple degeneracy stored as three pairs becomes one
     subspace of three bands). A flagged pair whose two bands are farther apart in energy than
     the grouping threshold \texttt{-gethr} (default $0.01$~Ry $\approx 0.14$~eV) is treated as non-degenerate and
     dropped, so a distant band is never pulled into the subspace.
   \item[Phase 2 --- form zones.] Adjacent flagged k-points sharing the same degenerate band
     group are joined into a connected \emph{zone}; a flagged point with no flagged neighbour is
     an \emph{isolated} single-point zone.
   \item[Phases 3--4 --- reference and propagation.] Within each zone the subspace is aligned to a
     clean (non-degenerate) reference neighbour with an SVD/Procrustes rotation, and the rotation
     is propagated across the zone by breadth-first search, with the holonomy correction and
     iterative gauge refinement described in section \nameref{sec:degenrot}.
   \item[Phase 5 --- output.] A per-zone report is printed and the zone map is saved to
     \emph{basisrotation\_zones.npy}.
 \end{description}

 Only the flagged k-points are ever modified; clean neighbours are read as reference anchors but
 never changed. The rotated wavefunctions overwrite the \emph{.wfc} files in place
 (there is no \emph{.wfc1} file, as in version 1). Both collinear and non-collinear calculations are supported: for
 the non-collinear case the same $N\times N$ unitary is applied to both spinor components.
...
\begin{table}[ht]
 ...
 \hline
  \texttt{-h}, \texttt{--help}  &\hspace*{2cm} Show a help message and exit \\
  \texttt{Mb}                   & Maximum band to consider (positional) \\
  \texttt{-np}                  & Number of processes to use (default: 1) \\
  \texttt{-gethr Ry}            & Energy-coherence threshold for grouping\\
                                & flagged bands into one subspace (default: 0.01~Ry;\\
...
The highest band has to be given because in principle not all bands that were initially calculated will
be complete, and it doesn't make sense to use them in further calculations.

The wave functions originally are saved for each k-point in a file, and the file contains the value of the wavefunction in each
...
of the same band as a function of k-points, which is suitable to differentiate.
\begin{equation*}
 \Psi_{\pmb{k}}(\pmb{r}) \rightarrow \Psi_{\pmb{r}}(\pmb{k})
\end{equation*}

Will save the wavefunctions of each band in a file \emph{wfcpos\#.npy} where \# = number of band.
By default the gradient files \emph{wfcgra\#.npy} are no longer written: the \texttt{geometry}
script computes the k-space gradient on the fly from \emph{wfcpos\#.npy}.
The option \texttt{-savegra} restores writing them.
Note that \texttt{geometry} requires uncompressed \emph{wfcpos} files, so \texttt{-c} should
not be used if \texttt{geometry} will be run afterwards.
...
\hline
  ...
  \texttt{-c}                   & Compress the output files \\
  \texttt{-savegra}             & Also write the \emph{wfcgra\#.npy} files (default: not written) \\
  \texttt{-o file\_path}        & Name of output log file\\
                                & Extension will be .log regardless of user input\\
  \texttt{-flush}               & Flush the output to stdout \\
  \texttt{-v}                   & Increase output verbosity \\
...
Depending on the \texttt{-prop} selection, \texttt{geometry} writes some of the following files
to the \emph{data/geometry} directory: \emph{berryConn\#\_\#.npy} and \emph{berryCur\#\_\#.npy}
(the Berry connection and curvature for each pair of bands, for \texttt{conn}/\texttt{curv}/\texttt{both}),
\emph{berryCur\#\_\#\_curl.npy} (curl-method curvature, for \texttt{chern\_curl}), and
\emph{chern\_number.npy}, \emph{chern\_number\_curl.npy}, \emph{chern\_number\_bp.npy},
\emph{chern\_number\_bp\_bz.npy} (the Chern number computed by the corresponding method).
...
\begin{tabular}[]{lr}
  \hline
  \texttt{-h}, \texttt{--help}  &\hspace*{2cm} Show a help message and exit \\
  \texttt{-np}                  & Number of threads to use (default: all cores) \\
  \texttt{-mb}                  & Minimum band to consider (default: 0) \\
  \texttt{-mem GB}              & Memory budget in GB for the streaming buffers (default: 32) \\
  \texttt{--no-regularize}      & Disable seam regularization. By default the connection/curvature\\
                                & is interpolated over the clustering-flagged dp-broken seam\\
                                & k-points (\emph{data/dp\_interp\_mask.npy}) because the finite-\\
                                & difference gradient spikes there; this flag writes the raw values\\
  \texttt{-prop}                & Property, or comma-separated list of properties, to calculate:\\
                                & both, conn, curv, chern, chern\_curl, chern\_bp, chern\_bp\_bz,\\
                                & plus the aliases chern\_all (all four Chern methods)\\
                                & and all (everything) (default: both)\\
  \texttt{-force}               & Recompute the Berry connections/curvatures even if all their\\
                                & output files already exist (by default the calculation is\\
                                & skipped when they do)\\
  \texttt{-d}                   & Decimal places to save the Chern number with (default: 0) \\
  \texttt{-o file\_path}        & Name of output log file\\
                                & Extension will be .log regardless of user input\\
  \texttt{-flush}               & Flush the output to stdout \\
  \texttt{-v}                   & Increase output verbosity \\
...
To calculate the linear conductivity, the script \texttt{conductivity} should be used
and for the second harmonic conductivity, the script \texttt{shg}.
\begin{verbatim}
...
\hline
  ...
  \texttt{-brd}                 & Energy broadening in Ry units (default: 0.01) \\
  \texttt{-o file\_path}        & Name of output log file\\
                                & Extension will be .log regardless of user input\\
  \texttt{-flush}               & Flush the output to stdout \\
  \texttt{-v}                   & Increase output verbosity \\
...
These scripts calculate the conductivity between 0 and the maximum energy (in Ry),
with a step (energy step) and a broadening.

The results are written to the files \texttt{sigmar.dat} and \texttt{sigmai.dat}
(real and imaginary parts of the linear conductivity;
\texttt{sigma2r.dat} and \texttt{sigma2i.dat} for the SHG):
the first column is the photon energy in eV and the remaining columns are the
tensor components in Rydberg atomic units ($e^2=2$, $\hslash=1$, lengths in bohr).
Since the k-point mesh is generated in {\sc Quantum Espresso} \texttt{tpiba} units
($2\pi/a$), the leftover powers of $k$ in each response
($d-2$ for the linear conductivity, $d-3$ for the SHG, in $d$ dimensions)
are converted internally to $\text{bohr}^{-1}$ using the lattice parameter,
so the output does not depend on the \texttt{tpiba} convention.
...
\chapter{Installing for noncollinear calculations}
\label{ch:noncolinear}

For noncollinear calculations, the fortran script \verb|wfck2r.f90| has to be edited,
...
\end{verbatim}
So this is immediately after writing the wavefunction (first of the spinor),
...
\end{verbatim}
and it's done.
You should see a link to the executable \verb|wfck2rFR| on the \verb|bin| directory of {\sc Quantum ESPRESSO}.
...
\begin{itemize}
 ...
 \item generatewfc.py
 \item degenrotation.py
 \item dotproduct.py
 \item clustering\_bands0.py
 \item basisrotation.py
 \item r2k.py
 \item berry\_geometry.py
 \item conductivity.py
 \item shg.py
\end{itemize}\medskip
...
\begin{itemize}
  ...
  \item jit.py
  \item \_logger.py
  \item wfck2rFR.f90 (Fortran source for the wfck2rFR.x utility, used in noncollinear calculations)
\end{itemize}
...
\begin{sloppypar}
The output is a set of files that will be used by other programs (\textit{bandsfinal.npy}, \textit{signalfinal.npy}, \textit{correct\_signalfinal.npy}, \textit{degeneratefinal.npy}, \textit{final\_score.npy} and \textit{completed\_bands.npy}), plus a human-readable \textit{final.report}; they are described below.
\end{sloppypar}
...
For clarity, the equations in this appendix are written for the two-dimensional case; the
implementation handles one-, two- and three-dimensional k-grids, where each k-point has $2d$
neighbors ($d$ the dimension), so the factors and maxima written as $4$ below are $2d$ in general.
...
(by neighbor we mean that the k-points that are implicit in the variables are adjacent in $x$ or $y$ directions in reciprocal space);
...
This graph is undirected; each edge carries the dot product (mapped to the metric $1-\frac{2}{\pi}\arccos\left|\braket{p}{q}\right|$) as its weight, which is used by the community detection described later.

\begin{equation}
    E = \left\{(p,q)\;|\;p,q\in P \;\;\wedge\;\; \left|\braket{p}{q}\right| > \mathrm{TOL}\right\}
\end{equation}

A cross-band edge (one whose endpoints have different original band indices $n$) is subject to an
anti-bridge guard: it is kept only if band $n_p$ has lost its own continuity across that link
(its same-band dot product is below $\mathrm{TOL}$), which is what happens at a genuine band crossing.
Otherwise the edge is a spurious bridge that would fuse two distinct bands (e.g.\ a degenerate pair)
into a single component, and it is dropped --- the same-band edge already carries the continuity.
...
Then, clustering these components gives larger components, that will form a new state that will be used as an initial state of the next iteration.
...
In practice the partition is not always the plain set of connected components: on the first iteration, and on every iteration that follows an error-region rebuild of the graph, the graph is partitioned with the Louvain community-detection method (weighted by the edges' dot products) under an adaptive resolution search, and any component holding more nodes than the number of k-points is re-split the same way.
...
We call clusters the components with many points that can not join to another large component, because they are geometrically incompatible.
These clusters are electronic bands of eigenenergies with part of their points attributed.
...
\begin{enumerate}
    ...
    \item It is symmetric, $\mathcal{S}(\mathrm{X}, \mathrm{Y}) = \mathcal{S}(\mathrm{Y}, \mathrm{X})$
...
We use the convolution of the Sobel operator, $(G_x, G_y)$, with the cluster's k-space projection $A$ to determine their edges.
...
In the implementation the energy weight is refined to $w_E = (1-\alpha)\,w_e$ (the dot product carrying $1-w_E$), where $w_e = \mathrm{clip}(\Delta_\text{gap}/\sigma,\, 0.1,\, 1)$ down-weights $f_p(E_q)$ when the inter-band gap $\Delta_\text{gap}$ has collapsed below the one-step dispersion $\sigma$ (spin--orbit / Kramers manifolds), where energy continuity cannot separate adjacent bands; for well-separated bands $w_e=1$ and the expression above is recovered exactly.
Moreover, the whole score $s_p^q$ (dot-product term included) is zeroed when $f_p(E_q)=0$ (a gross energy jump, see below) or when the cross-band anti-bridge veto fires (the candidate's band still overlaps itself strongly across the link, mirroring the graph-construction guard).

The value of $\alpha$ is a relaxation parameter in the optimization process.
Its initial value is $1$ (so the first iterations rely on the dot product alone, apart from the two vetoes just described, which are always active), and it is swept down to its minimum value of $0$ in steps of size \texttt{step} (default $0.5$, giving the sweep $1 \to 0.5 \to 0$).
The sweep is convergence-paced: $\alpha$ is held at its current value while the solution keeps improving and is only lowered after it stalls for a few consecutive iterations, so lowering $\alpha$ gradually shifts trust from the dot product toward energy continuity.

The function $f_p(E_q)$ scores the energy continuity between the energy $E_\text{best}$ that the cluster's trajectory predicts at $q$'s k-point and the candidate energy $E_q$. It is a smooth, scale-aware match with a gross-jump veto:

\begin{equation}
    f_p(E_q) = \left\{
    \begin{array}{cl}
        0, & \Delta E_\text{best}^q > E_\text{accept} \\[0.2cm]
        \dfrac{\sigma}{\sigma + \Delta E_\text{best}^q}, & \text{otherwise}
    \end{array}
    \right.
    \hspace{1.0cm} \Delta E_a^b = |E_a-E_b|
\end{equation}

where $\sigma$ is the local one-step energy dispersion (the $99.9$th percentile of $|E(k+1)-E(k)|$) and $E_\text{accept} = \max(3\sigma,\, \Delta_\text{gap}/2)$ is a loose gross-jump guard ($\Delta_\text{gap}$, the median spacing between adjacent bands, enters only as a loose floor, never as a sharp width): a candidate on the far side of an energy wall (a genuine inter-band gap) scores $0$ and is never joined.
This replaces an earlier objective, $\text{min}\{\Delta E_\text{best}^{i}\}_i / \Delta E_\text{best}^q$, which scored how close $E_q$ was to being the \emph{globally nearest} band; that measure was non-directional and became unreliable when the gap collapses below $\sigma$ (spin--orbit / Kramers manifolds), where the nearest band is no longer the continuous one.
...
The walk that collects the fitting points is truncated at the first cliff (an energy step larger than $E_\text{accept}$, or a change of original band index), so the polynomial is never fitted across a discontinuity, and the prediction is clipped to the local dispersion window $\pm\sigma$ around the anchor point.
If there are not enough continuous points to fit the curve, the score falls back to comparing the candidate energy directly with the reference point's own energy (in the assignment score) or with the nearest neighbor's energy (in the validation stage).
The gross-jump guard $E_\text{accept}$ uses the band spacing only as the loose floor above, never as a sharp penalty width: gating on the gap once shattered the graph, so energy is kept out of the graph edges and used only through $f_p(E_q)$ and the a-posteriori repair described below.
...
Hence, all the samples are compared to all clusters to obtain their similarities (this scoring is distributed over the available processes). The classification is made in rounds: the samples are joined to their best cluster in descending order of score, and a round stops as soon as it would join a second sample to a cluster already modified in that round. Then only the scores against the modified clusters are recomputed; unmodified clusters keep their cached scores.

This UML algorithm is the SA's bottleneck in executing time. Although the scoring inside each round is parallelized, the rounds themselves are sequential (each round depends on the joins of the previous one).

Ultimately, the samples are assigned to clusters, leading to a band solution (a sample may remain unassigned if no compatible cluster remains).
However, as mentioned before, many problematic points exist.
In particular, high energy bands usually have many oscillations, and in this case the assumption of smooth variation is inaccurate, and the metric criteria has to be improved.
...
The full SA is deterministic because the result is always the same for the same conditions. However, after one SA iteration (GT and UML methods), it obtains one solution for the band-crossing problem.
Different iterations obtain different number of bands completed and correct, which we want to maximize.
Bands may have points wrongly attributed to them, and so a verification and evaluation is needed.

We use two stages to evaluate the iteration result.
The first one considers only the dot-product information.
It identifies and signals points according to their assignment as indicated in table \ref{tab:signals1}.
...
\begin{table}[H]
    ...
    \begin{tabular}{ccc}
        ...
        $2$    & Degenerate        &                                            \\
        $3$    & Potential mistake & $0.2 < \overline{\left|\braket{p}{q}\right|} < 0.8$  \\
        $4$    & Potential correct & $0.8 < \overline{\left|\braket{p}{q}\right|} \leq 0.9 $ \\
        $5$    & Correct           & $\overline{\left|\braket{p}{q}\right|} > 0.9$           \\
        $6$    & Forced            & force-filled by the completeness pass                   \\\hline
    \end{tabular}
\end{table}

This stage signals some points that require more analysis, namely the ones that are signaled as potentially correct or mistaken.
The second stage of evaluation analyzes these points to decide if they are effectively correct or not.
(In-loop, only these doubtful points are analyzed; on the final pass, after the solver stops, every attributed point is re-validated --- see the a-posteriori repair section.)
...
\begin{table}[H]
    ...
    \begin{tabular}{ccc}
        ...
        $3$    & Other             & $0<D<4$  \\
        $4$    & Correct           & $D = 4$           \\
        $5$    & Forced            & force-filled by the completeness pass  \\\hline
    \end{tabular}
...
The points signaled as correct form well-constructed bands' portions: they are connected by same-band energy-continuity edges and survive intact as components.
The points not attributed, the mistakes, and the doubtful points receive no continuity edges; instead, every band's node at the k-points that carry an error originates fresh dot-product edges (an error-region-only rebuild), so the error region can re-cluster and re-attach to the good bands at its boundary, and a connectivity guard ensures no node is left isolated.
...
\subsection{A-posteriori repair and final classification}

When the iterative solver stops, its best result may still contain points that failed the energy-continuity test or that were left unattributed. Four passes then run, in this order, on that best result. The energy-based passes (1--3) never touch a point that already validated cleanly; the wavefunction-evidence pass (4) may, by design.

\begin{enumerate}
    \item \textbf{Energy-continuity repair.} The flagged (wrong / not-solved) points, points holding a valid but energy-discontinuous band, and energy-continuity outliers (points whose energy departs from the band's trusted trajectory even though the attribution is formally valid) are freed and jointly reassigned. Each empty slot's expected energy is extrapolated by a quadratic fit built from \emph{trusted} neighbors only, and the freed bands are redistributed to the slots they best match, never reusing a band twice at the same k-point, and only when the reassignment stays within the gross-jump guard $E_\text{accept}$. Repaired points become trusted, so the fix propagates inward, one layer per round.
    \item \textbf{Force completion.} Any slot still empty is force-filled with the closest available band in energy, using the continuity extrapolation as a reference. These points are marked \emph{Forced} (signal $6$ in table \ref{tab:signals1}, $5$ in table \ref{tab:signals2}); they are not genuine solves and are flagged as such.
    \item \textbf{Final revalidation, degenerate-group completion and bijection.} Every attributed slot is re-validated by the energy-continuity criteria (in-loop only the doubtful points are checked, which could let one side of a label swap ship as clean); an isolated failure on a band whose overall score exceeds $0.96$ is graded as a partial discontinuity rather than a mistake. Inside a near-degenerate group of bands (energies within a few mRy) the individual band label is a gauge choice, so the group's slots are completed by a within-group bijection. The full attribution is then made a per-k permutation (each original band used in at most one slot): duplicated bands keep their best copy and the losers move to a missing band by best energy fit. (The degenerate-group completion and the bijection also run as part of every in-loop validation, not only here.)
    \item \textbf{Wavefunction-evidence pass.} The energy criteria cannot see a label swap between near-parallel bands (a few-mRy error passes every energy gate), so cells whose attribution has no dot-product support are exchanged when the swap is dot-product-favoured, and the zero-support cells that remain are demoted, so that a silently swapped band cannot be reported clean.
\end{enumerate}

Finally each band is graded in the \textit{final.report} into one of: \texttt{CLEAN} (pristine), \texttt{USABLE} (no genuine errors; may carry a sub-pristine score or benign force-fills inside a degenerate doublet), \texttt{ROTATE} (usable after a basis rotation), \texttt{CHECK} (a force-fill across a real energy gap --- verify), or \texttt{FAIL} (a genuine energy break or a low score). The \texttt{CLEAN}/\texttt{USABLE}/\texttt{ROTATE} bands are the ones saved in \textit{completed\_bands.npy}.


\section{Bands Clustering program (BCP)}

The python program \textit{clustering\_bands0.py} implements the steps needed to use the algorithm explained previously. This program reads and prepares all the dependencies to perform the bands clustering algorithm, which is implemented in the \textit{clustering\_libs.py} module.
...
\begin{table}[H]
    ...
    \begin{tabular}{lcr}
        ...
        Name of the output log file & -o & \textit{cluster}\\
        Flush the output to stdout & -flush & \\
        Increase output verbosity & -v & \\
...
\bibitem{Bjorck1973}
\r{A}.~Bj\"{o}rck, G.~H. Golub,
  Numerical methods for computing angles between linear subspaces,
  Mathematics of Computation 27~(123) (1973) 579--594.
\newblock \href{https://doi.org/10.1090/S0025-5718-1973-0348991-3}
  {\path{doi:10.1090/S0025-5718-1973-0348991-3}}.
...
\newblock \href{https://doi.org/10.1137/10078356X}
  {\path{doi:10.1137/10078356X}}.
